function [labels,clustFilt,percElements] = function_pairingOfClusterElements(type,datanorm4Cluster,numC,visible,outdir)
...
suffix = outdir;%'ACd_IPs_61cells';
if ~exist(suffix,'dir'), mkdir(suffix); end
...
% %      print(gcf,'-depsc',file);
   ...
   title(sprintf('%g%% elements clustered',percElem));
   file=[suffix,'/pairwise',num2str(numClusters),'clusterReordered_',type,'_final'];
   print(gcf,'-djpeg',file);
   print(gcf,'-depsc',file);
